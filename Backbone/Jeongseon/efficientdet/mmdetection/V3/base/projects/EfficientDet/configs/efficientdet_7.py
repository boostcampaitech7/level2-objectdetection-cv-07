...
image_size = 640  # EfficientDet-B7은 더 큰 입력 이미지 크기를 사용합니다.
batch_augments = [
...
model = dict(
    ...
    neck=dict(
        ...
        out_channels=128,  # 더 큰 BiFPN 출력 채널
        start_level=0,
        norm_cfg=norm_cfg),
    bbox_head=dict(
        ...
        num_classes=80,
        num_ins=5,
        in_channels=128,  # BiFPN 출력 채널에 맞게 조정
        feat_channels=128,
        stacked_convs=3,
        norm_cfg=norm_cfg,
...
optim_wrapper = dict(
    ...
    clip_grad=dict(max_norm=10, norm_type=2)) # 이 부분 여기요 여기 사람있어요
