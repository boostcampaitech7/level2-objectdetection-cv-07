...
from transformers import AutoImageProcessor, AutoModelForObjectDetection, Trainer, AutoConfig
...
def main(exp_conf_path):
    ...
    if (conf['backbone_name'] != None) and conf['image_size_match_backbone']:
        baseline = timm.create_model(conf['backbone_name'], pretrained=False)
        baseline_config = timm.data.resolve_model_data_config(baseline)
        conf['image_size'] = baseline_config['input_size'][1]
    ...
    if (conf['backbone_name']):
        model_config = AutoConfig.from_pretrained(conf['model_name'], backbone=conf['backbone_name'], 
                                                id2label=id2label, label2id=label2id)
    else: 
        model_config = AutoConfig.from_pretrained(conf['model_name'], 
                                                id2label=id2label, label2id=label2id)
        
    model = AutoModelForObjectDetection.from_pretrained(conf['model_name'], 
                                                        config=model_config, 
